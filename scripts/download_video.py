...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Download Douyin video by URL")
    ...
    if not output_path:
        ...
        # Extract ID if possible from URL, else verify timestamp
        video_id = "douyin_video_" + str(int(time.time()))
        match = re.search(r'/video/(\d+)', video_url)
        if match:
            video_id = match.group(1)
        output_path = f"{video_id}.mp4"
    
    asyncio.run(download_video(video_url, output_path))
